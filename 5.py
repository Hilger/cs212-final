...
def test_darts():
    ...
    print double_out(170) #== ['T20', 'T20', 'DB']
    print double_out(171) #== None
    print double_out(100) #in (['T20', 'D20'], ['DB', 'DB'])
...
def double_out(total):
    ...
    def target_name(score):
        if score == 0:
            return None
        elif score == 50:
            return "DB"
        elif score % 2 == 0 and score <= 40:
            return "D" + str(score / 2)
        elif score == 25:
            return "SB"
        elif score % 3 == 0:
            return "T" + str(score / 3)
        else:
            return "S" + str(score)
    ...
    def list_all():
        ...
        for i in range(1,21):
            ...
            scores.add(i*2)
            scores.add(i*3)
        return [0] + sorted(list(scores), reverse=True)

    def valid_targets(target_list):
        return sum(target_list) == total and target_list[-1] % 2 == 0 \
        and (target_list[-1] <= 40 or target_list[-1] == 50) \
        and target_list[-1] != 0

    for score1 in list_all():
        for score2 in list_all():
            for score3 in list_all():
                final_targets = [score1, score2, score3]
                if valid_targets(final_targets):
                    return [target_name(i) for i in final_targets \
                    if target_name(i) is not None]
...
def outcome(target, miss):
    ...
    def adjacency_graph():
        order = "20 1 18 4 13 6 10 15 2 17 3 19 7 16 8 11 14 9 12 5".split()
        graph = {}

        for index, item in enumerate(order):
            graph[item] = [order[index-1]]
            graph[item].append(order[(index+1)%len(order)])

        return graph

    def partial_section(_target, base_prob):
        modifier = _target[0]
        section = _target[1:]
        section_graph = {}

        section_graph[_target] = base_prob - base_prob * miss
        if section == "B":
            return section_graph

        for adjacent in adjacency_graph()[section]:
            section_graph[modifier+adjacent] = (base_prob * miss) / 2

        return section_graph

    def all_section(modifier, base_prob):
        order = "20 1 18 4 13 6 10 15 2 17 3 19 7 16 8 11 14 9 12 5".split()
        section_graph = {}

        for section in order:
            section_graph[modifier + section] = base_prob / 20

        return section_graph

    modifier, section = target[0], target[1:]
    prob_dist = {}
    adjacencies = adjacency_graph()

    if section == "B":
        if modifier == "S":
            prob_dist.update(partial_section(target, 1 - miss))
            prob_dist.update(all_section("S", (1-miss) * miss + 1 * miss - 
                                               ((1-miss) * miss) / 4))
            prob_dist["DB"] = (((1 - miss) * miss) / 4)
        elif modifier == "D":
            miss = miss * 3 if miss * 3 <= 1 else 1
            prob_dist.update(partial_section(target, 1 - miss))
            prob_dist.update(all_section("S", (1-miss) * miss + 1 * miss - 
                                               ((1-miss) * miss) / 3))
            prob_dist["SB"] = (((1 - miss) * miss) / 3)
    else:
        if modifier == "T":
            prob_dist.update(partial_section(target, 1 - miss))
            prob_dist.update(partial_section("S" + section, miss))
        elif modifier == "D":
            prob_dist.update(partial_section(target, 1 - miss))
            prob_dist.update(partial_section("S" + section, 
                            miss / 2))
            prob_dist.update({"OFF" : miss / 2})
        elif modifier == "S":
            miss /= 5
            prob_dist.update(partial_section(target, 1 - miss))
            prob_dist.update(partial_section("D" + section, 
                            miss / 2))
            prob_dist.update(partial_section("T" + section, 
                            miss / 2))
...
def best_target(miss):
    ...
    all_targets = ["SB", "DB", "OFF"] + \
                  ["T" + str(i) for i in range(1,21)] + \
                  ["D" + str(i) for i in range(1, 21)] + \
                  ["S" + str(i) for i in range(1, 21)]

    score_dic = {}
    modifier_dic = {"S":1, "D":2, "T":3}

    for target in all_targets:
        if target != "OFF":
            modifier = target[0]
            section = int(target[1:]) if target[1:] != "B" else 25

            score_dic[target] = modifier_dic[modifier] * section
        else:
            score_dic[target] = 0
    ...
    for target in all_targets:
        ...
        _outcome = outcome(target, miss).iteritems()
        for k,v in _outcome:
            score += score_dic[k] * v
