...
def poly(coefs):
    ...
    for coef in reversed(list(coefs)):
        name_str = ""
        if coef:
            if not exponent:
                name_str = str(coef)
                ...
                if exponent == 1:
                    ...
                    name_str = "x**%d" % exponent
            elif exponent == 1:
                name_str = "%s * x" % (coef)
            elif exponent > 1:
                name_str = "%s * x**%d" % (coef, exponent)

        exponent -= 1
        if name_str:
            poly_str.append(name_str)

    p.__name__ = ' + '.join(poly_str)
...
def is_poly(x):
    ...
    return hasattr(x, 'coefs') # Is this cheating?
...
def add(p1, p2):
    ...
    for coef in p1.coefs:
        new_coefs.append(coef)

    for exponent, coef in enumerate(p2.coefs):
        if exponent > len(new_coefs)-1:
            new_coefs.append(coef)
        else:
            new_coefs[exponent] += coef
...
def mul(p1, p2):
    "Return a new polynomial which is the product of polynomials p1 and p."

    new_coefs = {}
    for exponent1, coef1 in enumerate(p1.coefs):
        for exponent2, coef2 in enumerate(p2.coefs):
            if exponent1+exponent2 in new_coefs:
                new_coefs[exponent1+exponent2] += coef1*coef2
            else:
                new_coefs[exponent1+exponent2] = coef1*coef2

    final_coefs = []
    for exponent in range(0, max(new_coefs.keys()) + 1):
        if exponent in new_coefs:
            final_coefs.append(new_coefs[exponent])
        else:
            final_coefs.append(0)

    return poly(final_coefs)
...
"""
If your calculus is rusty (or non-existant),print here is a refresher:
...
def deriv(p):
    ...
    for exponent, coef in enumerate(p.coefs[1:], start=1):
        new_coefs.append(coef * exponent)
...
def integral(p, C=0):
    ...
    for exponent, coef in enumerate(p.coefs, start=1):
        new_coefs.append(coef/exponent)
