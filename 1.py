...
def bowling(balls):
    ...
    frames = 0
    while balls:
        compute_score.append(balls.pop(0))
        if len(compute_score) == 3 and compute_score[0] == 10:
            score += sum(compute_score)
            frames += 1
            compute_score.pop(0)
        elif len(compute_score) == 3 and sum(compute_score[:2]) == 10:
            score += sum(compute_score)
            frames += 1
            compute_score = compute_score[2:]
        elif len(compute_score) >= 2 and sum(compute_score[:2]) < 10:
            score += sum(compute_score[:2])
            frames += 1
            compute_score = compute_score[2:]
    if frames < 10:
        return score + sum(compute_score)
    return score
...
def test_bowling():
    ...
    print 190 == bowling([9,1] * 10 + [9]) #199
    print 300 == bowling([10] * 12) #320
    print 200 == bowling([10, 5,5] * 5 + [10]) #210
